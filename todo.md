...
## 🇺🇸 NEXT — Highway 1 south, date TBD

Route is **built and validated**: 786.6 km over 3 days, GPX in `gpx/pch_*.gpx`,
write-up in **[BIG-SUR-GPX.md](./BIG-SUR-GPX.md)**. What's left is not routing:

> **Freshness gate:** the Limekiln/Pfeiffer/Refugio temperatures, dewpoint
> spreads, road controls, fire status, campground inventory and train space
> below were checked for a superseded early-August window. Refresh all of them
> once the new departure dates are known; none is a current booking or forecast.

- `[ ]` **CONFIRM a legal overnight for the new date.** Limekiln on ReserveCalifornia, **800-444-7275**, previously stated that there were no first-come-first-served campsites; recheck its reopening, hike/bike rule, price and inventory. Tentless roadside sleep is still camping, not a loophole. Dispersed camping is relevant only on verified-open **Los Padres National Forest** land, not Highway 1 right-of-way, state-park or private ranch land, and live fire/forest orders can close it. **Campfires are banned until 31 Jan 2027** under the recorded Forest Order; recheck whether a gas stove is allowed and obtain a California Campfire Permit if required.
...
- `[ ]` **For any future full Leggett attempt, load the official direction-specific USBR 95 files.** The [official collection](https://ridewithgps.com/collections/11043144) currently contains the [North-Central southbound file](https://ridewithgps.com/routes/46641093) but omits its matching official [northbound file](https://ridewithgps.com/routes/46362440). Use the northbound track from the Golden Gate Bridge to the CA-1/US-101 junction at Leggett (**352.77 km**) and the southbound track home (**352.40 km**): **705.17 km**, plus the Fell Street connector, with no US-101 before the endpoint. The completed 12–13 August ride turned around near Manchester and does not prove that the Leggett files were loaded. For the built SF→SD stages only, the current GPXs sit within 100 m of USBR 95 for 83.0% of day 1, 83.4% of day 2, 77.2% of day 3 and 64.5% of day 4 (median offset 0 m); re-measure with `python3 scripts/validate_usbr95_alignment.py`. Camp options and exclusions are in **[HIKER-BIKER-CAMPSITES.md](./HIKER-BIKER-CAMPSITES.md)**.
- `[x]` **Goleta is DECIDED — day 3 now uses the Obern Trail**, like the official USBR 95 does. **+4.3 km for +9.5 km of separated bike path**, 5.4 km less arterial, zero extra climb; day 3 is now **209.9 km**. Caveats: parts of the boardwalk are `surface=wood` and slick when damp, and you hit it early on a foggy morning; a shared-use path is slower than an arterial. If you're chasing a train, load [`pch_day3_alt_arterials.gpx`](./gpx/pch_day3_alt_arterials.gpx) (205.7 km) instead. **This is a different place from the 391 m illegal section** at km 13.7 in Winchester Canyon, which nothing avoids — the official route clips it too.
...
- `[x]` **GPX built** — 3 stages (786.6 km) + the Pfeiffer day-1/day-2 pair + day-3 arterials alternative + waypoint files; legality, surface, continuity and out-and-back spurs all audited (`python3 scripts/validate_pch_routes.py` → ALL CHECKS PASS). The first draft carried 62 km of via-point spurs; those are gone and a spur check now guards against them.
