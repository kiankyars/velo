...
2. **Routing** — [`scripts/build_routes.py`](./scripts/build_routes.py) downloads
   ...
   to ≤ 150 m for clean device rendering. For a phone you usually want a *small*
   file: [`scripts/gpx_from_here.py LAT LON`](./scripts/gpx_from_here.py) emits a
   Douglas-Peucker-simplified GPX (every turn kept, ~few-thousand points) from any
   live position to the end of a leg — e.g.
   [`gpx/ev15_from_karlsruhe.gpx`](./gpx/ev15_from_karlsruhe.gpx) (day-2 start,
   ~4.6k pts) imports cleanly into Ride with GPS / OsmAnd.
